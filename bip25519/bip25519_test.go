...
func mustDerivePub(src hdw.PublicKey, index uint32) hdw.PublicKey {
	k, err := src.Derive(index)
	if err != nil {
		panic(err)
	}
...
var edTestDataHMAC = []testChain{
	{
		...
		keys: []testKeyData{
			...
			{
				path:    []uint32{0 | hdw.BIP32Hard, 1, 2 | hdw.BIP32Hard},
			...
			{
				path:    []uint32{0 | hdw.BIP32Hard, 1, 2 | hdw.BIP32Hard, 2},
			...
			{
				path:    []uint32{0 | hdw.BIP32Hard, 1, 2 | hdw.BIP32Hard, 2, 1000000000},
	...
	},
	{
		...
		keys: []testKeyData{
			...
			{
				path:    []uint32{0, 2147483647 | hdw.BIP32Hard},
			...
			{
				path:    []uint32{0, 2147483647 | hdw.BIP32Hard, 1},
			...
			{
				path:    []uint32{0, 2147483647 | hdw.BIP32Hard, 1, 2147483646 | hdw.BIP32Hard},
			...
			{
				path:    []uint32{0, 2147483647 | hdw.BIP32Hard, 1, 2147483646 | hdw.BIP32Hard, 2},
	...
	// from BOLOS
	{
		...
		keys: []testKeyData{
			{
				path:    []uint32{4294967295 | hdw.BIP32Hard},
			...
			{
				path:    []uint32{44 | hdw.BIP32Hard, 148 | hdw.BIP32Hard, 0, 0},
			...
			{
				path:    []uint32{44 | hdw.BIP32Hard, 148, 1, 2},
	...
	},
	{
		...
		keys: []testKeyData{
			{
				path:    []uint32{2147483616, 391769539, 2147483613 | hdw.BIP32Hard},
	...
	},
	{
		...
		keys: []testKeyData{
			{
				path:    []uint32{2147483617 | hdw.BIP32Hard},
...
func TestBIP25519Derive(t *testing.T) {
	...
	key1 = mustDerive(root, 0|hdw.BIP32Hard).(*PrivateKey)
...
func TestBIP25519DeriveParallel(t *testing.T) {
	...
	path := []uint32{0, 1, 2, 1000}
	root := NewKeyFromSeed(seed, &Options{Mode: ModeRetry})

	// derive private
	key1 := root
	for _, x := range path {
		key1 = mustDerive(key1, x).(*PrivateKey)
	}

	// derive public
	key2 := root.ExtendedPublic().(*PublicKey)
	for _, x := range path {
		key2 = mustDerivePub(key2, x).(*PublicKey)
	}
