...
var testData = []curveTestData{
	{
		...
		chain: []testChain{
			...
			// BOLOS test suite
			{
				...
				keys: []testKeyData{
					{
						path:    []uint32{738197632, 335544448 | hdw.BIP32Hard, 0, 0},
					...
					{
						path:    []uint32{738197504, 335544320 | hdw.BIP32Hard, 16777216, 33554432},
		...
		chain: []testChain{
			{
				...
				keys: []testKeyData{
					...
					{
						path:    []uint32{28578 | hdw.BIP32Hard},
					...
					{
						path:    []uint32{28578 | hdw.BIP32Hard, 33941},
...
func TestEDDSAParallelDerivation(t *testing.T) {
	...
	// derive private key
	priv := hdw.PrivateKey(root)
	for _, index := range path {
		priv, err = priv.Derive(index)
		require.NoError(t, err)
	}

	// derive public key
	pub := root.ExtendedPublic()
	for _, index := range path {
		pub, err = pub.Derive(index)
		require.NoError(t, err)
	}
